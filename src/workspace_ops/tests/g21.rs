...
#[test]
fn branch_merge_fast_forward_updates_lock() {
    let temp = TempDir::new("branch-merge-ff");
    let backend = crate::git::Git2Backend::new();
    let _fixture = init_one_member_workspace(temp.path(), &backend, "branch-merge-ff-source");
    let member = temp.path().join("remote");
    let base = backend.head(&member).unwrap().commit.unwrap();
    backend
    ...
    let source_commit =
        commit_file(&member, "README.md", "source\n", "source", &[base_oid]).unwrap();
    ...
    let repos = response.repos.unwrap();
    let repo = repos.single();
    assert_eq!(repo.result, crate::BranchActionResult::Merged);
    assert_eq!(repo.source_ref.as_deref(), Some("feature/source"));
    assert_eq!(repo.target_branch.as_deref(), Some("main"));
    assert_eq!(
        repo.resulting_commit.as_deref(),
    ...
    assert_eq!(
        backend.head(&member).unwrap().commit,
...
#[test]
fn branch_merge_true_merge_commit_updates_lock() {
    let temp = TempDir::new("branch-merge-commit");
    let backend = crate::git::Git2Backend::new();
    let _fixture = init_one_member_workspace(temp.path(), &backend, "branch-merge-commit-source");
    let member = temp.path().join("remote");
    let base = backend.head(&member).unwrap().commit.unwrap();
    backend
        .branch_create(&member, "feature/source", "HEAD")
    ...
    backend.switch_branch(&member, "feature/source").unwrap();
    let base_oid = git2::Oid::from_str(&base).unwrap();
    commit_file(&member, "source.txt", "source\n", "source", &[base_oid]).unwrap();
    backend.switch_branch(&member, "main").unwrap();
    let local_commit = commit_file(&member, "local.txt", "local\n", "local", &[base_oid]).unwrap();

    let response = handle_branch(
        &backend,
        temp.path(),
        merge_request("feature/source"),
        "op_branch_merge_commit",
    )
    .unwrap();
    ...
    let merge_commit = repo
        .resulting_commit
        .as_ref()
        .expect("merge commit reported");
    assert_ne!(merge_commit, &local_commit);
    let observed = backend.head(&member).unwrap().commit.unwrap();
    assert_eq!(&observed, merge_commit);
    assert_eq!(
        read_lock(temp.path()).unwrap().members["mem_remote"].commit,
        Some(observed)
    );
...
#[test]
fn branch_merge_conflict_leaves_merge_head_and_reports_conflicted() {
    let temp = TempDir::new("branch-merge-conflict");
    let backend = crate::git::Git2Backend::new();
    let _fixture = init_one_member_workspace(temp.path(), &backend, "branch-merge-conflict-source");
    let member = temp.path().join("remote");
    let base = backend.head(&member).unwrap().commit.unwrap();
    backend
        .branch_create(&member, "feature/source", "HEAD")
    ...
    commit_file(&member, "README.md", "source\n", "source", &[base_oid]).unwrap();
    backend.switch_branch(&member, "main").unwrap();
    let local_commit = commit_file(&member, "README.md", "local\n", "local", &[base_oid]).unwrap();

    let response = handle_branch(
        &backend,
        temp.path(),
        merge_request("feature/source"),
        "op_branch_merge_conflict",
    )
    .unwrap();

    let repos = response.repos.unwrap();
    let repo = repos.single();
    assert_eq!(repo.result, crate::BranchActionResult::Conflicted);
    assert_eq!(repo.conflict_paths, vec!["README.md"]);
    assert_eq!(
        response.response.meta.aggregate_status,
...
#[test]
fn branch_merge_rejects_existing_merge_state_before_mutation() {
    let temp = TempDir::new("branch-merge-state");
    let backend = crate::git::Git2Backend::new();
    let _fixture = init_one_member_workspace(temp.path(), &backend, "branch-merge-state-source");
    let member = temp.path().join("remote");
    let before = backend.head(&member).unwrap().commit.unwrap();
    backend
        .branch_create(&member, "feature/source", "HEAD")
        .unwrap();
    fs::write(member.join(".git/MERGE_HEAD"), format!("{before}\n")).unwrap();

    let error = handle_branch(
        &backend,
        temp.path(),
        merge_request("feature/source"),
        "op_branch_merge_state",
    )
    .unwrap_err();

    assert_eq!(error.code, ErrorCode::InvalidRequest);
    assert_eq!(backend.head(&member).unwrap().commit, Some(before));
}

#[test]
fn branch_merge_rejects_dirty_member_before_mutation() {
    let temp = TempDir::new("branch-merge-dirty");
    let backend = crate::git::Git2Backend::new();
    let _fixture = init_one_member_workspace(temp.path(), &backend, "branch-merge-dirty-source");
    let member = temp.path().join("remote");
    let before = backend.head(&member).unwrap().commit.unwrap();
    backend
        .branch_create(&member, "feature/source", "HEAD")
